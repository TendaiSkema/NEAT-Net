...
class Node:
    def __init__(self, id, initial_threshold=0.1):
        self.id = id
        self.input_cons = []
        self.output_cons = []
    ...
    def process(self, inputs=[]):
        ...
        #print(f"Update {self.id} with inputs: {inputs}, weights: {self.weights}, threshhold: {self.threshold}, result: {self.int_val} | {self.spike}")
    ...
    def update_weights(self, inputs, global_error):
        for i, in_node_id in enumerate(self.input_cons):
            ...
            if inputs[i] and not self.spike and (global_error > 0):
                weight_change = 0.01 * global_error
            ...
            self.weights[i] = np.clip(self.weights[i], -1000.0, 1000.0)

    def stdp_update(self, last_spike_times):
        for i, in_node_id in enumerate(self.input_cons):
            pre_spike_time = last_spike_times[i]

            # Ensure pre_spike_time is valid (i.e., has spiked)
            if pre_spike_time == -np.inf:
                continue  # Skip update if pre-synaptic neuron hasn't spiked

            # Calculate delta_t (post - pre), skip if post has not spiked yet
            if self.last_spike_time != -np.inf:
                delta_t = self.last_spike_time - pre_spike_time

                # Modulate the STDP update by the global error
                if delta_t > 0:
                    weight_change = self.stdp_ltp(delta_t)
                else:
                    weight_change = self.stdp_ltd(delta_t)

                self.weights[i] += weight_change
                self.weights[i] = np.clip(self.weights[i], -VALUE_CAP, VALUE_CAP)

                print(f"Weight update: Node {self.id} - Weight {i}: {self.weights[i]} after change {weight_change}")
            else:
                # No valid spike time, skip or handle accordingly
                print(f"No valid post-synaptic spike time for Node {self.id}, skipping weight update for input {in_node_id}")

    def stdp_ltp(self, delta_t):
        # Long-term potentiation (LTP) function
        A_plus = 0.01  # Learning rate for LTP
        tau_plus = 5.0  # Time constant for LTP
        return A_plus * np.exp(-delta_t / tau_plus)

    def stdp_ltd(self, delta_t):
        # Long-term depression (LTD) function
        A_minus = -0.01  # Learning rate for LTD
        tau_minus = 5.0  # Time constant for LTD
    ...
    def calculate_threshold(self, activation_prob):
        return max(1.362932 * np.power(activation_prob, 0.070175) + -1.299556, 0)
    ...
    def __str__(self) -> str:
        return f"SpikingNode({self.id}: threshold: {self.threshold}, in: {[str(n) for n in self.input_cons]} out: {[str(n) for n in self.output_cons]}, weights: {self.weights})"
...
class Layer:
    ...
    def process(self, dag):
        """
        Process the nodes in the layer, calculate outputs, and apply STDP.
        """
        for node_id in self.nodes:
            node = dag.get_node(node_id)
            input_values = [dag.processed_dict[in_node_id] for in_node_id in node.input_cons]
            input_values = np.array(input_values)
            node.process(input_values.T)
            dag.processed_dict[node_id] = node.spike



            # Apply STDP after processing each node
            spike_times = [dag.get_node(in_node_id).last_spike_time for in_node_id in node.input_cons]
            node.stdp_update(spike_times)
    ...
    def process(self, dag):
        ...
        for node_id in self.nodes:
            ...
            for in_node_id in node.input_cons:
                if in_node_id in dag.processed_dict:
                    input_values.append(dag.processed_dict[in_node_id])
                else:
                    input_values.append(self.previous_outputs[in_node_id])  # Use the last iteration's value

            input_values = np.array(input_values)
            node.process(input_values.T)
            dag.processed_dict[node_id] = node.out_val
        ...
        for node_id in self.nodes:
            self.previous_outputs[node_id] = dag.processed_dict[node_id]
...
class DAG: 
    ...
    def process(self, inputs, iterations=10, return_spiketrains=False):
        ...
        for _ in range(iterations):
            ...
            for layer in self.processing_order:
                layer.process(self)  # Delegate processing to the Layer or MLayer
            ...
            for input_pattern, target in zip(inputs, targets):
                ...
                for layer in self.processing_order:
                    for node_id in layer:
                        node = self.get_node(node_id)
                        input_values = [self.processed_dict[in_node_id] for in_node_id in node.input_cons]
